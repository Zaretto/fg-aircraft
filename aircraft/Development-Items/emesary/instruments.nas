...
var ARA63Recipient =
{
    new: func(_ident)
    {
        ...
        {
print("AC: receive ",notification.Type);
...
#
# We cannot decide if in range and just not respond as the AN/SPN 46 system may have
# differences on a per carrier - so it is upto the AN/SPN system to decide if we are within range
                if(notification.Channel == 2) # TODO: use the actual channel
...
#
# ARA 63 update: this needs to clear out all of the signals if not received within the timeout
# period. It also handles the civilian ILS (as does the ARA-63 receiver above, but both
# are needed as there could be no carrier
