...
var update_tanker_for_replay = func
{
    var in_replay = 0;
    ...
    for (var i = 0; i < 2; i += 1)
     {
        ...
        if (in_replay)
        {
            if (getprop("sim/ai-replay/"~tid~"/orientation/pitch-deg")!= nil)
            {
                setprop("ai/models/"~tid~"/orientation/pitch-deg",         getprop("sim/ai-replay/"~tid~"/orientation/pitch-deg"));
                setprop("ai/models/"~tid~"/orientation/roll-deg",          getprop("sim/ai-replay/"~tid~"/orientation/roll-deg"));
                setprop("ai/models/"~tid~"/orientation/true-heading-deg",  getprop("sim/ai-replay/"~tid~"/orientation/true-heading-deg"));
                setprop("ai/models/"~tid~"/position/altitude-ft",          getprop("sim/ai-replay/"~tid~"/position/altitude-ft"));
                setprop("ai/models/"~tid~"/position/latitude-deg",         getprop("sim/ai-replay/"~tid~"/position/latitude-deg"));
                setprop("ai/models/"~tid~"/position/longitude-deg",        getprop("sim/ai-replay/"~tid~"/position/longitude-deg"));
            }
        }
        else
        {
            if (getprop("ai/models/"~tid~"/valid"))
            {
                setprop("sim/ai-replay/"~tid~"/orientation/pitch-deg",        getprop("ai/models/"~tid~"/orientation/pitch-deg"));
                setprop("sim/ai-replay/"~tid~"/orientation/roll-deg",         getprop("ai/models/"~tid~"/orientation/roll-deg"));
                setprop("sim/ai-replay/"~tid~"/orientation/true-heading-deg", getprop("ai/models/"~tid~"/orientation/true-heading-deg"));
                setprop("sim/ai-replay/"~tid~"/position/altitude-ft",         getprop("ai/models/"~tid~"/position/altitude-ft"));
                setprop("sim/ai-replay/"~tid~"/position/latitude-deg",        getprop("ai/models/"~tid~"/position/latitude-deg"));
                setprop("sim/ai-replay/"~tid~"/position/longitude-deg",       getprop("ai/models/"~tid~"/position/longitude-deg"));
            }
        }
}
}
